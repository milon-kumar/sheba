...
<h5 class="card-title"><b>Doctor name: </b>{{$doctor->first_name.' '.$doctor->last_name}}</h5>
    ...
    <p> <b class="card-text">Degree: </b>{{$doctor->profile->degree}}</p>
    <p> <b class="card-text">Email: </b>{{$doctor->email}}</p>
    <p> <b class="card-text">Phone Number: </b>{{$doctor->profile->phone}}</p>
    <p> <b class="card-text">Joining Date: </b>{{$doctor->profile->join_date}}</p>
    <p> <b class="card-text">Address: </b>{{$doctor->profile->address}}</p>
    <p> <b class="card-text">Gender: </b>{{$gender}}</p>
    <a href="#" class="btn btn-info">Call Doctor</a>
</div>

<div class="col">
<div class="card justify-self-center mt-3" style="width: 18rem;" >
  <img src="{{asset('storage/products/'.$doctor->profile->image)}}" class="card-img-top" alt="...">
